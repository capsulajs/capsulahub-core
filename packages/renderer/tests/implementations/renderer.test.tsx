...
import { mockWorkspaceComponents } from './helpers/utils';
import { LayoutABC, ItemA, ItemB, ItemC } from './helpers/web-components';
...
test('Calling renderLayouts when no layouts available in configuration', async () => {
  mockWorkspaceComponents({});
  const renderService = new RendererService();
  return expect(renderService.renderLayouts()).rejects.toThrow(noLayoutsAvailable);
});
